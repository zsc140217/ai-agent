@echo off
REM Ollama 部署脚本 - Windows批处理版本
REM 用于快速部署 bge-reranker-v2-m3 重排序模型

echo ==========================================
echo Ollama 重排序模型部署脚本
echo ==========================================

REM 设置模型存储路径到E盘（避免占用C盘空间）
set OLLAMA_MODELS=E:\ollama-models
echo.
echo [配置] 模型存储路径: %OLLAMA_MODELS%
echo.
...
REM 检查模型是否已存在
ollama list | findstr "bge-reranker-v2-m3" >nul 2>nul
if %errorlevel% equ 0 (
    echo ✅ 模型已存在，跳过下载
) else (
    echo 开始下载模型...
...
curl -s http://localhost:11434/api/embeddings -d "{\"model\": \"bge-reranker-v2-m3\", \"prompt\": \"query: 北京住宿标准 document: 北京一类城市住宿标准500元\"}" > %TEMP%\ollama_test.json

if %errorlevel% equ 0 (
    echo ✅ 模型测试成功
    echo.
    echo 响应示例:
    type %TEMP%\ollama_test.json | more
) else (
    echo ❌ 模型测试失败
    pause
    exit /b 1
...
echo.
echo 重要提示：
echo 为了让模型永久存储在E盘，请设置系统环境变量：
echo 1. 右键"此电脑" - "属性" - "高级系统设置" - "环境变量"
echo 2. 新建系统变量：
echo    变量名: OLLAMA_MODELS
echo    变量值: E:\ollama-models
echo 3. 重启Ollama服务
echo.
echo 下一步：
