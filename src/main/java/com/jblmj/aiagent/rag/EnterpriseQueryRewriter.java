...
public class EnterpriseQueryRewriter {
    ...
    private static final String DOMAIN_KNOWLEDGE = """
        【企业差旅领域术语】
        - 城市分类：一类城市（北京、上海、深圳、广州、杭州、成都）、二类城市（省会城市、其他地级市）
        - 住宿标准：一类城市500元/晚、二类城市350元/晚
        - 交通标准：高铁二等座、飞机经济舱（行程超4小时可选飞机）
        - 补贴标准：伙食补助100元/天、市内交通补助80元/天
        - 报销规则：打车报销与交通补助二选一

        【同义词映射】
        - 城市别名：魔都=上海、帝都=北京、羊城=广州、蓉城=成都、鹏城=深圳
        - 城市缩写：BJ=北京、SH=上海、GZ=广州、SZ=深圳、HZ=杭州
        - 城市分类：一线城市=一类城市、二线城市=二类城市、省会=二类城市
        - 住宿相关：住宿=酒店=宾馆=旅馆、五星级=高档酒店、四星及以下=标准酒店
        - 费用相关：报销=费用=标准=预算=能报多少
        - 出差相关：出差=差旅=外勤=商务出行
        - 交通相关：坐车=交通方式、打车=出租车=网约车
        - 补助相关：补助=补贴=津贴
        """;

    // Few-shot 改写示例（扩展版：15个示例覆盖更多场景）
    private static final String FEW_SHOT_EXAMPLES = """
        ...
        示例11：复杂条件 → 保留逻辑
        原始："如果打车了还能领交通补助吗"
        改写："打车报销与市内交通补助关系 二选一规则"
        原因：保留条件逻辑，明确查询关系

        示例12：客户信息查询 → 明确实体
        原始："去杭州拜访某某科技公司的联系人是谁"
        改写："杭州某某科技公司客户信息 联系人"
        原因：明确实体（公司名）和查询目标（联系人）

        示例13：综合预算 → 拆解维度
        原始："明天去上海出差，住宿和伙食一共能报多少"
        改写："上海一类城市出差费用 住宿标准 伙食补助 总计"
        原因：拆解多个费用维度，保留计算意图
    ...
    public String rewrite(String originalQuery) {
        ...
        try {
            // 3. 调用LLM改写（Temperature=0.1，平衡稳定性和泛化能力）
            // 虽然实测0.0稳定性最高(95%)，但业界标准推荐0.1-0.3
            // 原因：0.1保留适度多样性，对未见过的查询有更好的泛化能力
            String rewrittenQuery = chatClient.prompt()
                    .user(rewritePrompt)
                    .options(org.springframework.ai.chat.prompt.ChatOptions.builder()
                            .temperature(0.1)  // 符合业界标准（Anthropic/LlamaIndex推荐）
                            .build())
                    .call()
                    .content();
    ...
    private String buildRewritePrompt(String originalQuery) {
        return String.format("""
                ...
                7. 识别城市别名和缩写，转换为标准地名
                8. 对比查询保留对比结构（A和B、A vs B）
                9. 数值计算查询明确计算意图（总计、合计）
                10. 条件查询提取条件逻辑（如果...那么...）
